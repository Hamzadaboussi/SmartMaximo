...
import { get_CurrentUsername } from '../Firebase/Firestore/FirebaseQuery';
...
export const handleRetrieveUsers = async (
    setUsers : React.Dispatch<React.SetStateAction<UserC[]>>,
    setFilteredUsers: React.Dispatch<React.SetStateAction<UserC[]>>

) => {
    const usersRef = firestore().collection('users');
    const token = await messaging().getToken();
    console.log("hedhatoken",token)
    usersRef
      .get()
      .then((querySnapshot) => {
        ...
        querySnapshot.forEach((documentSnapshot) => {
          ...
          if(documentSnapshot.data().deviceToken!=token)
          userList.push(user);
        });
        setUsers(userList);
